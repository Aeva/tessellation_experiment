...
void main()
{
	...
	vec3 Triangle[3] = {
		imageLoad(Sphere0, ivec2(InIndex, 0)).xyz,
		imageLoad(Sphere0, ivec2(InIndex, 1)).xyz,
		imageLoad(Sphere0, ivec2(InIndex, 2)).xyz
	};
	...
	for (int i=0; i<3; ++i)
	{
		imageStore(Sphere1, ivec2(OutIndex , i), vec4(Subtriangle[i], 1.0));
	}
}
